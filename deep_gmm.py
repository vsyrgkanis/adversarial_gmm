    ...
    def fit(self, data_z, data_p, data_y):
        ...
        if num_instruments > 1:
            ...
            for tree in range(cluster_labels.shape[1]):
                ...
                plt.scatter(data_z[:,0], data_z[:,1], c=[int(i % 23) for i in cluster_labels[:, tree]])
            plt.savefig('clusters3.png')
            if self._critic_type == 'Gaussian':
                plt.figure()
                z1 = np.linspace(np.min(data_z[:, 0]), np.max(data_z[:, 0]), 20)
                z2 = np.linspace(np.min(data_z[:, 1]), np.max(data_z[:, 1]), 20)
                z1v, z2v = np.meshgrid(z1, z2)
                for it, (ctr, prec) in enumerate(zip(center_grid, precision_grid)):
                    plt.contour(z1v, z2v, matplotlib.mlab.bivariate_normal(z1v, z2v, 
                        mux=ctr[0], muy=ctr[1], 
                        sigmax=1/prec, sigmay=1/prec), 1, c=int(it % 23))
